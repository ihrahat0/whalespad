-- Complete SQL Update for Project Details Page
-- Add all missing fields to store real project data

-- First, add missing columns to project_submissions table
...
ADD COLUMN IF NOT EXISTS features TEXT[], -- Array of key featuresn
...
ADD COLUMN IF NOT EXISTS slug TEXT UNIQUE; -- URL-friendly slug for routing
...
-- Create tokenomics table
CREATE TABLE IF NOT EXISTS project_tokenomics (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    project_id UUID REFERENCES project_submissions(id) ON DELETE CASCADE,
    name TEXT NOT NULL, -- e.g., 'Presale', 'Liquidity', 'Team'
    percentage INTEGER NOT NULL,
    amount BIGINT NOT NULL,
    color TEXT NOT NULL, -- Hex color for chart
    description TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS project_roadmap (
    ...
    phase TEXT NOT NULL, -- e.g., 'Development Phase'
...
DROP POLICY IF EXISTS "Allow authenticated insert/update on project_team_members" ON project_team_members;
DROP POLICY IF EXISTS "Allow authenticated insert/update on project_tokenomics" ON project_tokenomics;
DROP POLICY IF EXISTS "Allow authenticated insert/update on project_roadmap" ON project_roadmap;
DROP POLICY IF EXISTS "Allow authenticated insert/update on project_partnerships" ON project_partnerships;
...
-- Create policies for authenticated users (admins) to insert/update
CREATE POLICY "Allow authenticated insert/update on project_team_members" 
ON project_team_members FOR ALL TO authenticated USING (true);

CREATE POLICY "Allow authenticated insert/update on project_tokenomics" 
ON project_tokenomics FOR ALL TO authenticated USING (true);

CREATE POLICY "Allow authenticated insert/update on project_roadmap" 
ON project_roadmap FOR ALL TO authenticated USING (true);

CREATE POLICY "Allow authenticated insert/update on project_partnerships" 
ON project_partnerships FOR ALL TO authenticated USING (true);

-- Insert sample data for demonstration (replace with real data)
-- Sample project (you can modify this with real project data)
-- First, let's define a UUID for our sample project
DO $$
DECLARE
...
INSERT INTO project_submissions (
    ...
    'QuantumChain Protocol represents the next evolution in decentralized finance, combining quantum-resistant cryptography with cross-chain interoperability. Our platform enables secure, fast, and scalable transactions across multiple blockchain networks while providing advanced privacy features. Built with institutional-grade security and designed for mass adoption, QuantumChain is positioned to become the backbone of the future financial ecosystem.',
...
COMMENT ON VIEW project_details_view IS 'Complete project details with all related data for the frontend';

-- Final verification query
SELECT 'Database schema update completed successfully!' as message,
       'Tables created: project_team_members, project_tokenomics, project_roadmap, project_partnerships' as tables_created,
       'View created: project_details_view' as view_created; 