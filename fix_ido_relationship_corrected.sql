-- Fix IDO pools relationship with project_submissions (CORRECTED VERSION)
...
-- Add missing logo_url column to project_submissions if it doesn't exist
...
-- Verify the relationship works (with safe column access)
SELECT 
    ip.*,
    ps.project_name,
    ps.token_symbol,
    COALESCE(ps.logo_url, '') as logo_url
FROM ido_pools ip
