...
interface FloatingConnectWalletProps {
  onMobileMenuClose?: () => void;
}

export const FloatingConnectWallet: React.FC<FloatingConnectWalletProps> = ({ onMobileMenuClose }) => {
  ...
  useEffect(() => {
    const handlePendingWalletConnect = () => {
      console.log('🔗 Floating Wallet Connect: Global event received, showing modal in 200ms');
      setTimeout(() => {
        console.log('🔗 Floating Wallet Connect: Showing terms modal now');
  ...
  const handleClick = () => {
    if (isConnected) {
      ...
      if (onMobileMenuClose) {
        console.log('🔗 Floating Wallet Connect: Mobile menu detected, closing menu and dispatching event');
        onMobileMenuClose();
        // Dispatch global event after menu closes
        setTimeout(() => {
          console.log('🔗 Floating Wallet Connect: Dispatching wallet-connect-pending event');
          window.dispatchEvent(new CustomEvent('wallet-connect-pending'));
        }, 100);
      } else {
        console.log('🔗 Floating Wallet Connect: Desktop mode, showing modal directly');
        setShowTermsModal(true);
      }
    }
  };
  ...
  return (
    <>
      <motion.div
        className="floating-wallet-container"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.5 }}
      >
        <motion.button
          className="floating-wallet-btn"
          onClick={handleClick}
        ...
        >
          {isConnecting ? (
            <>
              <span className="floating-wallet-loading-spinner"></span>
